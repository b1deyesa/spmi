<x-layout.setting>
    <div class="flex justify-between mb-5">
        <h2 class="text-xl font-semibold text-gray-700">Tambah Program Studi</h2>
    </div>
    <form action="{{ route('setting.program-studi.store') }}" method="POST" class="text-sm flex flex-col gap-4">
        @csrf
        <x-input type="text" label="Nama Program Studi" name="nama" :required="true" />
        <x-input type="data-list" label="Jenjang" name="jenjang_id" :required="true" :options="$jenjangs" />
        <x-input type="data-list" label="Fakultas" name="fakultas_id" :required="true" :options="$fakultases" />
