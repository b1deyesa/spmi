        ...
        <tbody>
            @forelse ($programStudis as $programStudi)
                <tr>
                    ...
                    @foreach ($years as $year)
                        <td class="border-b border-zinc-300 px-2 py-1" align="center">@if ($jenjang_magister[$programStudi->id][$year]) {{ $jenjang_magister[$programStudi->id][$year] }} @else <span class="text-red-300">⊗</span> @endif</td>
                    @endforeach
                </tr>
            @empty
                <tr>
